...
import { Search, BarChart3, Settings, Info, User, LogOut, Plus } from 'lucide-react';
...
const navigationItems = [
  ...
  { id: 'dashboard', label: 'Dashboard', icon: BarChart3, href: '/dashboard?tab=dashboard' },
  { id: 'settings', label: 'Settings', icon: Settings, href: '/dashboard?tab=settings' },
  { id: 'about', label: 'About', icon: Info, href: '/about' },
];

export default function NavigationSidebar({ isOpen, activeTab, onTabChange, onClose }: NavigationSidebarProps) {
    ...
    const ctx = gsap.context(() => {
      if (isOpen) {
        ...
        .fromTo(
          navItemsRef.current,
          { opacity: 0, x: -20 },
          { opacity: 1, x: 0, duration: 0.3, stagger: 0.1, ease: 'power2.out', overwrite: true },
          '-=0.2'
        )
        .fromTo(
          ...
          >
            ...
            <div className="flex items-center gap-3 p-3 rounded-lg hover:bg-gray-50 transition-colors duration-200 cursor-pointer group">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-gray-900 truncate">Dr. John Doe</p>
                <p className="text-xs text-gray-500 truncate">john.doe@inforx.ng</p>
              </div>
              <LogOut className="w-4 h-4 text-gray-400 opacity-0 group-hover:opacity-100 transition-opacity duration-200" />
            </div>
          </div>
        </div>
