...
const MetricCard = ({ title, value, change, icon: Icon, color, delay = 0 }: MetricCardProps) => {
...
const HealthProgress = () => {
  ...
  return (
    <Card ref={progressRef} className="col-span-full lg:col-span-1">
      <CardHeader>
...
const RecentActivity = () => {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case "warning": return "bg-orange-100 text-orange-700 dark:bg-orange-900/50 dark:text-orange-400";
      case "info": return "bg-sky-100 text-sky-700 dark:bg-sky-900/50 dark:text-sky-400";
  ...
  return (
    <Card ref={activityRef} className="col-span-full lg:col-span-2">
...
const QuickActions = () => {
  ...
  const actions = [
    { label: "Upload Document", icon: FileText, href: "/dashboard/upload", color: "bg-sky-500" },
    { label: "AI Analysis", icon: Activity, href: "/dashboard/interpreter", color: "bg-emerald-500" },
    { label: "View Records", icon: Users, href: "/dashboard/records", color: "bg-purple-500" },
    { label: "Download Report", icon: Download, href: "#", color: "bg-orange-500" }
  ];

  return (
    <Card ref={actionsRef} className="col-span-full">
      <CardHeader>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {actions.map((action, index) => (
            <Button
              key={action.label}
              variant="ghost"
              className="h-auto p-4 flex flex-col items-center gap-2 hover:bg-slate-50 dark:hover:bg-slate-800 transition-all duration-200 group"
              asChild
            >
              <a href={action.href}>
                <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${action.color} group-hover:scale-110 transition-transform duration-200`}>
...
export default function ModernDashboardOverview() {
  ...
  const metrics = [
    { title: "Total Records", value: "24", change: 12, icon: FileText, color: "bg-sky-500" },
    { title: "AI Analyses", value: "18", change: 8, icon: Activity, color: "bg-emerald-500" },
    { title: "Health Score", value: "86%", change: 4, icon: Heart, color: "bg-red-500" },
    { title: "Appointments", value: "3", change: -1, icon: Calendar, color: "bg-purple-500" }
  ];
