...
const exampleTexts = {
  prescription:
    "Metformin 500mg - Take 1 tablet twice daily with meals for Type 2 diabetes management. Continue for 3 months then review.",
  lab_report:
    "Complete Blood Count: WBC: 12,500/μL (High), RBC: 4.2 million/μL (Normal), Hemoglobin: 10.5 g/dL (Low), Platelets: 180,000/μL (Normal)",
  medical_scan:
    "Abdominal Ultrasound: Liver appears normal in size and echogenicity. Gallbladder shows multiple small echogenic foci consistent with gallstones.",
  clinical_notes:
    "Patient presents with persistent headache, fatigue, and mild fever. No nausea or vomiting. Vital signs stable. Recommend rest and follow-up.",
};

export default function InterpreterInterface({
  onResult,
  onLoading,
  onError,
  isLoading,
    ...
    try {
      ...
      const response = await fetch("/api/ai", {
        ...
        body: JSON.stringify({
          ...
          language:
            state.language === "english" ? "English" : "Nigerian Pidgin",
      ...
      const result: InterpretationResult = {
        id: `result_${state.documentType}_${state.inputText.length}`,
    ...
    <div ref={formRef} className="h-full flex flex-col p-6 overflow-y-auto">
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              onChange={(e) =>
                setState((prev) => ({
                  ...prev,
                  language: e.target.value as "english" | "pidgin",
                }))
              }
      ...
      <div ref={submitRef}>
        <Button
          ...
          disabled={
            isLoading || (!state.inputText.trim() && !state.uploadedFile)
          }
